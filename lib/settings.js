...
const DEFAULTS = {
  ...
  // each platform is independently shown/hidden in the footer from the admin panel
  social: {
    instagram: { url: 'https://www.instagram.com/shopweaves1?stkn=MW1ib2V5dG93NHN4Mw==', on: true },
    facebook: { url: 'https://www.facebook.com/share/1HpErLoctd/', on: true },
    twitter: { url: '#', on: true },
    linkedin: { url: '#', on: true },
  },
...
function load() {
  ...
  merged.social = normalizeSocial(merged.social);
  return merged;
}

/** Migrates the old "social: { instagram: 'https://...' }" (plain URL) shape
 * saved by earlier versions into the current { url, on } per-platform shape. */
function normalizeSocial(social) {
  const out = {};
  Object.keys(DEFAULTS.social).forEach((key) => {
    const v = social && social[key];
    if (typeof v === 'string') out[key] = { url: v || '#', on: true };
    else if (v && typeof v === 'object') out[key] = { url: v.url || '#', on: v.on !== false };
    else out[key] = DEFAULTS.social[key];
  });
  return out;
}
...
function save(patch) {
  ...
  if (has('instagramUrl', 'facebookUrl', 'twitterUrl', 'linkedinUrl')) {
    const platform = (key) => ({
      url: String(p[key + 'Url'] || '').trim() || '#',
      on: p[key + 'On'] === '1',
    });
    next.social = {
